...
# the rest of this code is not usefull or is older version; so not commented
def testLinUCB():
	pass


def parseYahooFile(fileName):
	stats = []
	lineCount = 0
	numArticle = 0
	...
	with open(fileName, 'r') as f:
		...
		# 	line = f.readline()
			...
			# print user_features
			# print pool_articles
			# stats.append(len(pool_articles))
			# if len(pool_articles) != 20:
			# 	stats.append(articleID)

			# for x in pool_articles:
			# 	if x[0] not in articleDic:
			# 		articleDic[articleID] = True
			clicks = clicks + click

	print '# Distinct Articles ', len(articleDic.keys())
	print '# Articles :', lineCount, ' Clicks : ', clicks

	diff = time.time() - t1
	# print "time taken is : ", diff, " seconds."
	return stats, articleDic
...
def LinUCB(fileName):
	with open(fileName, 'r') as f:
		...
		for line in f:
			...
			# print line
			# print timeStart, article_chosen, click, user_features, pool_articles

			max_pta = 0
			max_article = -1

			randomChosen = int(random()*len(pool_articles))
			randomArticle = pool_articles[randomChosen][0]
			for article in pool_articles:
				# featureVector = np.concatenate((user_features[:-1], article[1:-1]), axis = 0)
				featureVector = user_features[:-1]
				if len(featureVector) is not d:
				...
				if article_id not in articles:
					articles[article_id] = LinUCBStruct(d)


				pta = np.dot(articles[article_id].theta, featureVector)

				pta = pta + alpha * np.sqrt(np.dot(np.dot(featureVector,articles[article_id].A_inv), featureVector))
				if pta < 0:
					print 'lalal'
				# choosing max arm
				if pta > max_pta:
			...
			if randomArticle == article_chosen:
				randomCtr = randomCtr + click
				randomNum = randomNum + 1
			

			if max_article==article_chosen:
				articles[article_chosen].accesses = True

				numArticlesChosen = numArticlesChosen + 1
				ctr = ctr + click

				articles[article_chosen].A = articles[article_chosen].A + np.outer(featureVector, featureVector)
				if click:
					articles[article_chosen].b = articles[article_chosen].b + click * featureVector
				# articles[article_chosen].A_inv = np.linalg.inv(articles[article_chosen].A)
				articles[article_chosen].updateInv()
				articles[article_chosen].updateTheta()
				# if numArticlesChosen < 2000 or numArticlesChosen%200==0:
				# 	for x in articles.keys():
				# 		articles[x].A_inv = np.linalg.inv(articles[x].A)
				# 		articles[x].accesses = False

				if numArticlesChosen%200==0:
					print 'total : ', totalArticles, ' ratio : ', (ctr*1.0/numArticlesChosen)/(randomCtr*1.0/randomNum), 'RdArticles : ', randomNum , ' rdCtr : ', randomCtr, ' UCBArt : ', numArticlesChosen, ' UCBCtr : ', ctr, 'numNoArt', countNoArticle
			elif max_article==-1:
				countNoArticle = countNoArticle + 1
	...
	with open(fileName, 'r') as f:
		...
		for line in f:
			...
			for article in pool_articles:
				...
				if article_id not in articles:
					articles[article_id] = GreedyStruct()

				pta = articles[article_id].CTR
			...
			if random() < eta:
				max_article = int(random()*len(pool_articles))
				max_article = pool_articles[max_article][0]
			...
			if max_article==article_chosen:
				...
				articles[article_chosen].accesses = articles[article_chosen].accesses + 1
				articles[article_chosen].clicks = articles[article_chosen].clicks + click
				articles[article_chosen].CTR = articles[article_chosen].clicks/articles[article_chosen].accesses

				if numArticlesChosen%500==0:
					print 'total : ', totalArticles, ' ratio : ', (ctr*1.0/numArticlesChosen)/(randomCtr*1.0/randomNum), 'RdArticles : ', randomNum , ' rdCtr : ', randomCtr, ' UCBArt : ', numArticlesChosen, ' UCBCtr : ', ctr, 'numNoArt', countNoArticle
					# print [articles[x].CTR for x in articles.keys()]
			elif max_article==-1:
				countNoArticle = countNoArticle + 1
